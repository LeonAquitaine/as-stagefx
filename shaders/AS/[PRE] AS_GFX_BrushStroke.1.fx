...
// --- Category: Stroke Shape & Position ---
static const float BRUSHSTROKE_HEIGHT_MIN = 0.01f;
static const float BRUSHSTROKE_HEIGHT_MAX = 1.0f;
static const float BRUSHSTROKE_HEIGHT_DEFAULT = 0.2f;
uniform float BandHeight < ui_type = "slider"; ui_label = "Stroke Height"; ui_tooltip = "Controls the nominal height/thickness of the brush stroke."; ui_min = BRUSHSTROKE_HEIGHT_MIN; ui_max = BRUSHSTROKE_HEIGHT_MAX; ui_step = 0.01f; ui_category = "Stroke Shape & Position"; > = BRUSHSTROKE_HEIGHT_DEFAULT;

static const float BRUSHSTROKE_ANGLE_MIN = -180.0f;
static const float BRUSHSTROKE_ANGLE_MAX = 180.0f;
static const float BRUSHSTROKE_ANGLE_DEFAULT = 0.0f;
uniform float BandAngle < ui_type = "slider"; ui_label = "Stroke Angle"; ui_tooltip = "Rotation angle of the brush stroke in degrees."; ui_min = BRUSHSTROKE_ANGLE_MIN; ui_max = BRUSHSTROKE_ANGLE_MAX; ui_step = 0.1f; ui_category = "Stroke Shape & Position"; > = BRUSHSTROKE_ANGLE_DEFAULT;

static const float BRUSHSTROKE_CENTER_X_MIN = 0.0f;
static const float BRUSHSTROKE_CENTER_X_MAX = 1.0f;
static const float BRUSHSTROKE_CENTER_X_DEFAULT = 0.5f;
uniform float BandCenterX < ui_type = "slider"; ui_label = "Stroke Center X"; ui_tooltip = "Horizontal center of the stroke (0.0 = left, 0.5 = center, 1.0 = right)."; ui_min = BRUSHSTROKE_CENTER_X_MIN; ui_max = BRUSHSTROKE_CENTER_X_MAX; ui_step = 0.01f; ui_category = "Stroke Shape & Position"; > = BRUSHSTROKE_CENTER_X_DEFAULT;

static const float BRUSHSTROKE_CENTER_Y_MIN = 0.0f;
static const float BRUSHSTROKE_CENTER_Y_MAX = 1.0f;
static const float BRUSHSTROKE_CENTER_Y_DEFAULT = 0.5f;
uniform float BandCenterY < ui_type = "slider"; ui_label = "Stroke Center Y"; ui_tooltip = "Vertical center of the stroke (0.0 = top, 0.5 = center, 1.0 = bottom)."; ui_min = BRUSHSTROKE_CENTER_Y_MIN; ui_max = BRUSHSTROKE_CENTER_Y_MAX; ui_step = 0.01f; ui_category = "Stroke Shape & Position"; > = BRUSHSTROKE_CENTER_Y_DEFAULT;

// --- Category: Edge & Ink Appearance ---
static const float BRUSHSTROKE_SHAPE_X_MIN = 0.1f;
static const float BRUSHSTROKE_SHAPE_X_MAX = 50.0f;
static const float BRUSHSTROKE_SHAPE_X_DEFAULT = 5.0f;
uniform float StrokeShapeX < ui_type = "slider"; ui_label = "Ink Pattern Scale X (Along Stroke)"; ui_tooltip = "FBM noise scale along stroke. Smaller values = longer, more stretched ink features. Larger = more condensed features."; ui_min = BRUSHSTROKE_SHAPE_X_MIN; ui_max = BRUSHSTROKE_SHAPE_X_MAX; ui_step = 0.1f; ui_category = "Edge & Ink Appearance"; > = BRUSHSTROKE_SHAPE_X_DEFAULT;

static const float BRUSHSTROKE_SHAPE_Y_MIN = 1.0f;
static const float BRUSHSTROKE_SHAPE_Y_MAX = 100.0f;
static const float BRUSHSTROKE_SHAPE_Y_DEFAULT = 40.0f;
uniform float StrokeShapeY < ui_type = "slider"; ui_label = "Ink Pattern Scale Y (Across Stroke)"; ui_tooltip = "FBM noise scale across stroke. Larger values = finer, denser ink details/fibers. This, with Scale X, controls anisotropy."; ui_min = BRUSHSTROKE_SHAPE_Y_MIN; ui_max = BRUSHSTROKE_SHAPE_Y_MAX; ui_step = 0.1f; ui_category = "Edge & Ink Appearance"; > = BRUSHSTROKE_SHAPE_Y_DEFAULT;

static const float BRUSHSTROKE_FEATHER_MIN = 0.0001f;
static const float BRUSHSTROKE_FEATHER_MAX = 0.2f; 
static const float BRUSHSTROKE_FEATHER_DEFAULT = 0.01f;
uniform float StrokeFeather < ui_type = "slider"; ui_label = "Ink Edge Feathering"; ui_tooltip = "Softness of the ink edges defined by thresholding FBM noise."; ui_min = BRUSHSTROKE_FEATHER_MIN; ui_max = BRUSHSTROKE_FEATHER_MAX; ui_step = 0.0001f; ui_category = "Edge & Ink Appearance"; > = BRUSHSTROKE_FEATHER_DEFAULT;

static const float BRUSHSTROKE_CONTRAST_MIN = 0.5f;
static const float BRUSHSTROKE_CONTRAST_MAX = 5.0f;
static const float BRUSHSTROKE_CONTRAST_DEFAULT = 2.0f;
uniform float InkContrast < ui_type = "slider"; ui_label = "Ink Noise Contrast"; ui_tooltip = "Sharpens the FBM noise (power function exponent). Higher values create more defined, less blurry ink patterns."; ui_min = BRUSHSTROKE_CONTRAST_MIN; ui_max = BRUSHSTROKE_CONTRAST_MAX; ui_step = 0.1f; ui_category = "Edge & Ink Appearance"; > = BRUSHSTROKE_CONTRAST_DEFAULT;

static const float BRUSHSTROKE_THRESH_EDGE_MIN = 0.0f;
static const float BRUSHSTROKE_THRESH_EDGE_MAX = 1.0f;
static const float BRUSHSTROKE_THRESH_EDGE_DEFAULT = 0.55f;
uniform float MinInkThresholdAtEdge < ui_type = "slider"; ui_label = "Min Ink Threshold (Edge)"; ui_tooltip = "FBM noise must exceed this value to appear as ink near the stroke's nominal edge. Higher = more broken up edge."; ui_min = BRUSHSTROKE_THRESH_EDGE_MIN; ui_max = BRUSHSTROKE_THRESH_EDGE_MAX; ui_step = 0.01f; ui_category = "Edge & Ink Appearance"; > = BRUSHSTROKE_THRESH_EDGE_DEFAULT;

static const float BRUSHSTROKE_THRESH_CENTER_MIN = 0.0f;
static const float BRUSHSTROKE_THRESH_CENTER_MAX = 1.0f;
static const float BRUSHSTROKE_THRESH_CENTER_DEFAULT = 0.35f;
uniform float MinInkThresholdAtCenter < ui_type = "slider"; ui_label = "Min Ink Threshold (Center)"; ui_tooltip = "FBM noise must exceed this value to appear as ink at the stroke's center. Should be <= Edge threshold for solid core."; ui_min = BRUSHSTROKE_THRESH_CENTER_MIN; ui_max = BRUSHSTROKE_THRESH_CENTER_MAX; ui_step = 0.01f; ui_category = "Edge & Ink Appearance"; > = BRUSHSTROKE_THRESH_CENTER_DEFAULT;

static const float BRUSHSTROKE_EXTEND_MIN = 0.0f;
static const float BRUSHSTROKE_EXTEND_MAX = 0.5f;
static const float BRUSHSTROKE_EXTEND_DEFAULT = 0.2f;
uniform float EdgeExtendFactor < ui_type = "slider"; ui_label = "Edge Extend Factor"; ui_tooltip = "How far (relative to half stroke height) ink can 'bleed' or 'splatter' beyond the nominal stroke height."; ui_min = BRUSHSTROKE_EXTEND_MIN; ui_max = BRUSHSTROKE_EXTEND_MAX; ui_step = 0.01f; ui_category = "Edge & Ink Appearance"; > = BRUSHSTROKE_EXTEND_DEFAULT;
...
static const float BRUSHSTROKE_SHADOW_OPACITY_DEFAULT = 0.0f; // Disabled by default
uniform float ShadowOpacity < ui_type = "slider"; ui_label = "Shadow Opacity"; ui_tooltip = "Opacity of the drop shadow. 0.0 disables the shadow."; ui_min = BRUSHSTROKE_SHADOW_OPACITY_MIN; ui_max = BRUSHSTROKE_SHADOW_OPACITY_MAX; ui_step = 0.01f; ui_category = "Shadow Settings"; > = BRUSHSTROKE_SHADOW_OPACITY_DEFAULT;
...
static const float BRUSHSTROKE_SHADOW_ANGLE_DEFAULT = -45.0f;
...
static const float BRUSHSTROKE_SHADOW_DIST_DEFAULT = 0.005f;
...
// --- Category: Color & Shading (Main Stroke) ---
uniform float3 StrokeColor < ui_type = "color"; ui_label = "Stroke Color"; ui_category = "Color & Shading (Main Stroke)"; > = float3(0.1f, 0.1f, 0.1f); 

static const float BRUSHSTROKE_SHADOW_INT_MIN = 0.0f; // Renaming for clarity: StrokeTextureShadowIntensity
static const float BRUSHSTROKE_SHADOW_INT_MAX = 1.0f;
static const float BRUSHSTROKE_SHADOW_INT_DEFAULT = 0.3f;
uniform float StrokeTextureShadowIntensity < ui_type = "slider"; ui_label = "Ink Texture Shadow Intensity"; ui_tooltip = "Darkens parts of the ink based on FBM noise values, adding texture to the main stroke."; ui_min = BRUSHSTROKE_SHADOW_INT_MIN; ui_max = BRUSHSTROKE_SHADOW_INT_MAX; ui_step = 0.01f; ui_category = "Color & Shading (Main Stroke)"; > = BRUSHSTROKE_SHADOW_INT_DEFAULT;

static const float BRUSHSTROKE_HIGHLIGHT_INT_MIN = 0.0f; // Renaming for clarity: StrokeTextureHighlightIntensity
static const float BRUSHSTROKE_HIGHLIGHT_INT_MAX = 1.0f;
static const float BRUSHSTROKE_HIGHLIGHT_INT_DEFAULT = 0.2f;
uniform float StrokeTextureHighlightIntensity < ui_type = "slider"; ui_label = "Ink Texture Highlight Intensity"; ui_tooltip = "Brightens parts of the ink based on FBM noise values, adding texture to the main stroke."; ui_min = BRUSHSTROKE_HIGHLIGHT_INT_MIN; ui_max = BRUSHSTROKE_HIGHLIGHT_INT_MAX; ui_step = 0.01f; ui_category = "Color & Shading (Main Stroke)"; > = BRUSHSTROKE_HIGHLIGHT_INT_DEFAULT;

uniform bool UseSceneColorForBackground < ui_label = "Use Scene Color for Background"; ui_tooltip = "If checked, uses the original scene color behind the stroke. If unchecked, uses a solid background color."; ui_category = "Color & Shading (Main Stroke)"; > = true;
uniform float3 BackgroundColorSolid < ui_type = "color"; ui_label = "Solid Background Color (if not using scene)"; ui_category = "Color & Shading (Main Stroke)"; > = float3(1.0f, 1.0f, 1.0f);
...
float GetStrokeAlpha(
    ...
    float current_band_center_x, float current_band_center_y, 
    float current_band_angle_deg, float current_band_height,
...
{    float current_band_half_height = current_band_height * 0.5f;
    ...
    float current_angle_rad = AS_radians(-current_band_angle_deg); 
    float s_rot = sin(current_angle_rad);
    float c_rot = cos(current_angle_rad);

    // Apply aspect ratio correction to texture coordinates before rotation
    float2 centered_tc = current_texcoord - float2(current_band_center_x, current_band_center_y);
...
float4 PS_BrushStroke(float4 pos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    float3 blended_background_color = UseSceneColorForBackground ? original_color.rgb : BackgroundColorSolid;
    ...
    {        float shadow_angle_rad = AS_radians(ShadowAngle); // Note: ShadowAngle is typically positive for down-right
        ...
        float shadow_casting_alpha = GetStrokeAlpha(
            ...
            BandCenterX, BandCenterY, BandAngle, BandHeight,
        ...
        {
            blended_background_color = lerp(blended_background_color, ShadowColor, shadow_casting_alpha * ShadowOpacity);
        }
    }

    // --- Main Stroke Pass ---
    float paper_alpha = GetStrokeAlpha(
        texcoord,
        BandCenterX, BandCenterY, BandAngle, BandHeight,
        StrokeShapeX, StrokeShapeY, InkContrast,
        MinInkThresholdAtCenter, MinInkThresholdAtEdge,
    ...
    float3 final_color = lerp(blended_background_color, modified_stroke_color, paper_alpha);
