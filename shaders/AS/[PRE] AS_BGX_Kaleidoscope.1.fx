...
// static const float AS_TWO_PI = 6.283185307179586f; // Provided by AS_Utils.1.fxh as AS_TWO_PI
// static const float AS_PI = 3.141592653589793f;     // Provided by AS_Utils.1.fxh as AS_PI
static const float EPSILON = 0.00001f; // Local epsilon, AS_Utils.1.fxh also has AS_EPSILON
...
uniform float FractalZoomPulseSpeed < ui_category="Animation - Fractal Pattern";
    ...
    ui_tooltip = "Speed of the fractal zoom pulsation (relative to Master Animation Speed).";
> = 0.5f;
...
uniform float PaletteCycleSpeedFactor < ui_category="Animation - Colors & Waves";
    ...
    ui_tooltip = "Additional speed control for how fast colors cycle in the palette. Multiplies Master Animation Speed. Original GLSL factor for time: 0.4";
> = 0.4f;

uniform float WaveMotionSpeedFactor < ui_category="Animation - Colors & Waves";
    ui_type = "drag"; ui_min = 0.0; ui_max = 2.0; ui_step = 0.01;
    ui_label = "Wave Motion Speed Factor";
    ui_tooltip = "Additional speed control for how fast the 'tendril' waves move. Multiplies Master Animation Speed. Original GLSL factor for time: 1.0";
> = 1.0f;
...
// --- Global UV Transform Animation ---
uniform float GlobalRotationSpeed < ui_category="Animation - Global Transform";
    ui_type = "drag"; ui_min = -1.0; ui_max = 1.0; ui_step = 0.01;
    ui_label = "Overall Rotation Speed";
    ui_tooltip = "Speed of overall rotation of the UV space (relative to Master Animation Speed).";
> = 0.0f;

uniform float GlobalPulsingZoomStrength < ui_category="Animation - Global Transform";
    ui_type = "drag"; ui_min = 0.0; ui_max = 0.5; ui_step = 0.01;
    ui_label = "Overall Pulsing Zoom Strength";
    ui_tooltip = "Strength of the overall pulsing zoom effect applied to the entire pattern.";
> = 0.0f;

uniform float GlobalPulsingZoomSpeed < ui_category="Animation - Global Transform";
    ui_type = "drag"; ui_min = 0.0; ui_max = 2.0; ui_step = 0.01;
    ui_label = "Overall Pulsing Zoom Speed";
    ui_tooltip = "Speed of the overall pulsing zoom effect (relative to Master Animation Speed).";
> = 0.5f;
...
float3 palette(float t) {
    // Using AS_TWO_PI from AS_Utils.1.fxh
...
float2 kaleidoscope_transform(float2 uv, int sectors) {
    ...
    // Using AS_PI from AS_Utils.1.fxh
...
float4 PS_KaleidoscopePattern(float4 vpos : SV_Position, float2 texcoord : TEXCOORD0) : SV_Target {
    // Correctly get master animated time using AS_Utils.
    ...
    float2 uv = (fragCoord * 2.0 - iResolution_xy) / iResolution_xy.y;

    // 1. Global UV Rotation
    if (abs(GlobalRotationSpeed) > EPSILON) { // Use local EPSILON or AS_EPSILON from AS_Utils
        float angle_rot = master_anim_time * GlobalRotationSpeed;
        float s = sin(angle_rot);
        float c = cos(angle_rot);
        // For row vector uv: uv' = mul(uv, M) with M = [c s; -s c] for CCW rotation
        float2x2 rot_matrix = float2x2(c, s, -s, c); 
        uv = mul(uv, rot_matrix); 
    }

    // 2. Global Pulsing Zoom
    ...
    if (KaleidoscopeSectors > 1) {
        uv = kaleidoscope_transform(uv, KaleidoscopeSectors);
    }
    ...
    for (int i_loop = 0; i_loop < 4; ++i_loop) {
        ...
        uv = frac(uv * current_fractal_zoom) - 0.5;
        ...
        d = abs(d);
        d = pow(0.01f / (d + EPSILON), 1.2f); // Local EPSILON or AS_EPSILON
        finalColor += col * d;
    }
...
//--------------------------------------------------------------------------------------
technique AnimatedPsychedelicKaleidoscope_V3 // Incremented version name
{
    pass
    {
