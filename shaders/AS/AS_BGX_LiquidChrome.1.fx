...
uniform float coord_scale < ui_type = "drag"; ui_min = 1.0; ui_max = 50.0; ui_step = 0.1; ui_label = "Pattern Detail Scale"; ui_tooltip = "Adjusts the overall size of the psychedelic patterns. Higher values zoom in, revealing finer details and faster perceived motion."; ui_category = "Pattern Shape"; > = 10.0f;

uniform int warp_iterations < ui_type = "drag"; ui_min = 1; ui_max = 20; ui_step = 1; ui_label = "Warp Complexity"; ui_tooltip = "Increases the intricacy and 'folding' of the core warped pattern. More iterations lead to deeper, more detailed distortions."; ui_category = "Pattern Shape"; > = 5;

// --- Color Pattern ---
uniform float len_post_warp_cos_factor < ui_type = "drag"; ui_min = 0.0; ui_max = 2.0; ui_step = 0.01; ui_label = "Color Band Frequency"; ui_tooltip = "Adjusts the density of color bands or concentric rings overlaying the pattern. Higher values create more, tighter bands."; ui_category = "Color Pattern"; > = 0.4f;

uniform float len_post_warp_subtract < ui_type = "drag"; ui_min = -20.0; ui_max = 20.0; ui_step = 0.1; ui_label = "Color Band Offset"; ui_tooltip = "Shifts the phase or starting point of the color banding/ring pattern, affecting color distribution."; ui_category = "Color Pattern"; > = 10.0f;

// --- Vertical Lines ---
uniform int stripe_iterations < ui_type = "drag"; ui_min = 0; ui_max = 10; ui_step = 1; ui_label = "Vertical Line Layers"; ui_tooltip = "Number of overlaid vertical stripe patterns. Each layer adds more lines with different spacing. Set to 0 to disable stripes."; ui_category = "Vertical Lines"; > = 5;

uniform float stripe_period_base < ui_type = "drag"; ui_min = 0.005; ui_max = 0.5; ui_step = 0.001; ui_label = "Vertical Line Spacing"; ui_tooltip = "Controls the fundamental spacing of the vertical line patterns. Smaller values create denser, more frequent lines."; ui_category = "Vertical Lines"; > = 0.09f;

uniform float stripe_scale < ui_type = "drag"; ui_min = 10.0; ui_max = 1000.0; ui_step = 1.0; ui_label = "Vertical Line Sharpness"; ui_tooltip = "Adjusts the sharpness or definition of the vertical lines. Higher values can make lines thinner and more pronounced."; ui_category = "Vertical Lines"; > = 200.0f;

// --- Color Tuning ---
uniform float color_phase_r < ui_type = "drag"; ui_min = -AS_TWO_PI; ui_max = AS_TWO_PI; ui_step = 0.01; ui_label = "Red Hue Cycle"; ui_tooltip = "Shifts the color palette by adjusting the cycle for the red channel. Experiment for different color schemes."; ui_category = "Color Tuning"; > = 0.2f;

uniform float color_phase_g < ui_type = "drag"; ui_min = -AS_TWO_PI; ui_max = AS_TWO_PI; ui_step = 0.01; ui_label = "Green Hue Cycle"; ui_tooltip = "Shifts the color palette by adjusting the cycle for the green channel."; ui_category = "Color Tuning"; > = 0.1f;

uniform float color_phase_b < ui_type = "drag"; ui_min = -AS_TWO_PI; ui_max = AS_TWO_PI; ui_step = 0.01; ui_label = "Blue Hue Cycle"; ui_tooltip = "Shifts the color palette by adjusting the cycle for the blue channel."; ui_category = "Color Tuning"; > = -0.05f;

uniform float color_r_multiplier < ui_type = "drag"; ui_min = 0.0; ui_max = 3.0; ui_step = 0.01; ui_label = "Red Intensity"; ui_tooltip = "Controls the overall intensity or brightness of the red color channel."; ui_category = "Color Tuning"; > = 1.15f;

uniform float background_gradient_strength < ui_type = "drag"; ui_min = 0.0; ui_max = 0.1; ui_step = 0.001; ui_label = "Distortion Tint Strength"; ui_tooltip = "Controls the intensity of a color tint that is mixed in based on the final warped coordinates."; ui_category = "Color Tuning"; > = 0.01f;

uniform float4 background_base_color < ui_type = "color"; ui_label = "Distortion Tint Color"; ui_tooltip = "Sets the color of the tint mixed in based on the final warped coordinates, affecting the overall image tone."; ui_category = "Color Tuning"; > = float4(0.05, 0.07, 0.10, 0.0);
...
uniform float warp_time_factor_x < ui_type = "drag"; ui_min = 0.0; ui_max = 0.5; ui_step = 0.001; ui_label = "Horizontal Flow Speed"; ui_tooltip = "Controls the speed of the horizontal animation or 'breathing' in the main warp pattern."; ui_category = AS_CAT_ANIMATION; > = 0.07f;

uniform float warp_offset_amp_x < ui_type = "drag"; ui_min = 0.0; ui_max = 1.0; ui_step = 0.01; ui_label = "Horizontal Flow Strength"; ui_tooltip = "Determines how much the pattern animates or 'breathes' along the horizontal axis."; ui_category = AS_CAT_ANIMATION; > = 0.2f;

uniform float warp_time_factor_y < ui_type = "drag"; ui_min = 0.0; ui_max = 0.5; ui_step = 0.001; ui_label = "Vertical Flow Speed"; ui_tooltip = "Controls the speed of the vertical animation or 'undulation' in the main warp pattern."; ui_category = AS_CAT_ANIMATION; > = 0.1f;
...
uniform int LiquidChrome_AudioTarget < ui_type = "combo"; ui_label = "Audio Target"; ui_tooltip = "Select which parameter will be modulated by audio"; ui_items = "None\0Flow Speed\0Color Cycle\0Pattern Scale\0"; ui_category = AS_CAT_AUDIO; > = 2;
...
float4 PS_LiquidChrome(float4 vpos : SV_Position, float2 texcoord : TEXCOORD) : SV_Target
{
    ...
    AS_DEPTH_EARLY_RETURN(texcoord, LiquidChrome_EffectDepth)    // Get animation time with speed and keyframe support
    ...
    return float4(AS_composite(result.rgb, _as_originalColor.rgb, LiquidChrome_BlendMode, LiquidChrome_BlendAmount), 1.0f);
}
