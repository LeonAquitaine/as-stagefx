...
#include "AS_Noise.1.fxh" // For AS_hash22, AS_hash21

namespace AS_StainedLights {
// ============================================================================
...
uniform float PatternScale < ui_type = "slider"; ui_label = "Pattern Scale"; ui_tooltip = "Overall scale of the stained glass pattern."; ui_min = PATTERN_SCALE_MIN; ui_max = PATTERN_SCALE_MAX; ui_step = PATTERN_SCALE_STEP; ui_category = AS_CAT_PATTERN; > = PATTERN_SCALE_DEFAULT;
uniform float EdgeCurve < ui_type = "slider"; ui_label = "Edge Curvature"; ui_tooltip = "Controls the sharpness of the edges within the pattern."; ui_min = EDGE_CURVE_MIN; ui_max = EDGE_CURVE_MAX; ui_step = EDGE_CURVE_STEP; ui_category = AS_CAT_PATTERN; > = EDGE_CURVE_DEFAULT;
uniform float EdgeBias < ui_type = "slider"; ui_label = "Edge Bias"; ui_tooltip = "Adjusts the overall brightness of the edges."; ui_min = EDGE_BIAS_MIN; ui_max = EDGE_BIAS_MAX; ui_step = EDGE_BIAS_STEP; ui_category = AS_CAT_PATTERN; > = EDGE_BIAS_DEFAULT;
uniform int Iterations < ui_type = "slider"; ui_label = "Pattern Iterations"; ui_tooltip = "Number of iterations for pattern layering. Higher values create more detail but may affect performance."; ui_min = ITERATIONS_MIN; ui_max = ITERATIONS_MAX; ui_category = AS_CAT_PATTERN; > = ITERATIONS_DEFAULT;
...
uniform float AnimationSpeed < ui_type = "slider"; ui_label = "Animation Speed"; ui_tooltip = "Controls the speed at which the pattern evolves. Set to 0 to pause animation and use keyframe only."; ui_min = ANIMATION_SPEED_MIN; ui_max = ANIMATION_SPEED_MAX; ui_step = ANIMATION_SPEED_STEP; ui_category = AS_CAT_ANIMATION; > = ANIMATION_SPEED_DEFAULT;

// --- Audio Reactivity ---
AS_AUDIO_UI(StainedLights_AudioSource, "Audio Source", AS_AUDIO_BEAT, AS_CAT_AUDIO)
AS_AUDIO_MULT_UI(StainedLights_AudioMultiplier, "Audio Intensity", AUDIO_MULTIPLIER_DEFAULT, AUDIO_MULTIPLIER_MAX, AS_CAT_AUDIO)
AS_AUDIO_TARGET_UI(StainedLights_AudioTarget, "None\0Animation Speed\0Pattern Scale\0Edge Curvature\0", AUDIO_TARGET_DEFAULT)

// --- Post Processing ---
uniform float CurveIntensity < ui_type = "slider"; ui_label = "Curve Intensity"; ui_tooltip = "Applies a power curve to the output color for contrast."; ui_min = CURVE_INTENSITY_MIN; ui_max = CURVE_INTENSITY_MAX; ui_step = CURVE_INTENSITY_STEP; ui_category = AS_CAT_FINAL; > = CURVE_INTENSITY_DEFAULT;
uniform float NoiseAmount < ui_type = "slider"; ui_label = "Noise Amount"; ui_tooltip = "Adds a subtle noise texture to the final output."; ui_min = NOISE_AMOUNT_MIN; ui_max = NOISE_AMOUNT_MAX; ui_step = NOISE_AMOUNT_STEP; ui_category = AS_CAT_FINAL; > = NOISE_AMOUNT_DEFAULT;
...
float4 generatePattern(float2 uv) {
    ...
    uv.x -= 0.5;
    float2 hashInput = floor(float2(uv.x - uv.y, uv.x + uv.y));
    float3 cid2 = float3(AS_hash22(hashInput), AS_hash21(hashInput));
...
float4 StainedLightsPS(float4 vpos : SV_Position, float2 texcoord : TEXCOORD0) : SV_Target
{
    ...
    float audioReactivity = AS_audioModulate(1.0, StainedLights_AudioSource, StainedLights_AudioMultiplier, true, 0);
    ...
    if (animSpeedFinal <= AS_MIN_NORM) {
        ...
        t = (AS_timeSeconds() * animSpeedFinal) + AnimationKeyframe;
    }
    ...
    // Add noise
    o.rgb += AS_hash21(texcoord * ReShade::ScreenSize.xy + AS_timeSeconds()) * NoiseAmount;
    o.a = 1.0; // Ensure alpha is 1 after all color operations

    // Blend with original scene
    float4 finalColor = float4(AS_composite(o.rgb, _as_originalColor.rgb, BlendMode, BlendStrength), 1.0);
...
technique AS_BGX_StainedLights < ui_label="[AS] BGX: Stained Lights"; ui_tooltip="Dynamic stained glass light patterns."; >
{
    pass
    {
        ...
        PixelShader = AS_StainedLights::StainedLightsPS;
    }
}
